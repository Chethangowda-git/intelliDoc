...
export default function ChatPanel({ document, onClose }) {
  ...
  async function handleSend(prefill) {
    ...
    try {
        ...
        for (const part of parts) {
          ...
          if (event === 'token' && data.text) {
            ...
            setMessages(prev => {
              const updated = [...prev];
              const last = updated[updated.length - 1];
              if (last?._streaming) { delete last._streaming; last.sources = data.sources || []; }
              return [...updated];
            });
            setStreaming(false);
            setStatusMsg('');
            ...
            setMessages(prev => {
              const updated = [...prev];
              const last = updated[updated.length - 1];
              if (last?._streaming) { last.content = data.message || 'Something went wrong.'; last._error = true; delete last._streaming; }
              return [...updated];
            });
            setStreaming(false);
            setStatusMsg('');
      ...
      setMessages(prev => {
        const updated = [...prev];
        const last = updated[updated.length - 1];
        if (last?._streaming) { last.content = 'Something went wrong. Please try again.'; last._error = true; delete last._streaming; }
        return [...updated];
      });
      setStreaming(false);
      setStatusMsg('');
  ...
  return (
    <div style={s.panel}>
      ...
      <div style={s.header}>
        <div style={s.headerLeft}>
          ...
          <div>
            ...
            <p style={s.docMeta}>{document.chunkCount > 0 && `${document.chunkCount} chunks indexed`}</p>
          </div>
        </div>
        ...
        {messages.length === 0 && !streaming && (
          <div style={s.emptyState}>
            ...
            <div style={s.suggestions}>
              {SUGGESTIONS.map(s_ => (
                <button key={s_} style={s.suggestionBtn} onClick={() => handleSend(s_)}>
                  {s_}
                </button>
              ))}
            </div>
      ...
      {/* Scroll to bottom */}
      {!atBottom && (
        <button style={s.scrollBtn} onClick={() => { setAtBottom(true); bottomRef.current?.scrollIntoView({ behavior: 'smooth' }); }}>
